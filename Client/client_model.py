...
class User:
    ...
    def request_batch(self, max_size):
        ...
        ratio, request_node = find_top_ratio(curr_root, None, 0)
        if ratio == sys.maxsize:  # the selected node has not been used yet
            # self.get_knn(request_node, 2)
            return request_node.song_id, 2, request_node.k_lower
        else:
    ...
    def evaluate_batch(self, like_data):
        ...
        ratio, root = find_top_ratio(self.root_list[self.root_index], None, 0)
        likes = 0
        for i in range(len(self.batch)):
            if like_data[i] == 1:
                likes += 1
                node = Node(self.batch[i].song_id)
                root.add_like()
                root.set_child(node)
                node.set_child(root)
            if like_data[i] == -1:
        ...
        if likes == 0 and len(self.batch) > 2:
            top_ratio, top_node = find_top_ratio(self.root_list[self.root_index], None, 0)
    ...
    def filter_batch(self, request_node, candidates):
        ...
        self.lower_bound += len(candidates) - len(batch)
        request_node.increment_k_bound(self.lower_bound)
        self.batch = batch
...
def find_top_ratio(self, root, curr_best_node, curr_best_ratio):
    # initial run
    if curr_best_ratio == 0:
        used_nodes = set()
        used_nodes.add(root)
        curr_best_node = root
    ...
    for child in root.get_neighbors():
        ...
        if len(child.get_neighbors()) > 0 and not used_nodes.issuperset({child}):
            ratio, new_best_node = find_top_ratio(child, curr_best_node, curr_best_ratio)
